          ...
          <template v-if="item.dropdown === true">
            <v-expansion-panels variant="accordion">
              <v-expansion-panel>
                ...
                >
                  <v-list-item :prepend-icon="item.icon" class="menu-item" :title="menuWidth > 200 ? item.title : ''"></v-list-item>
                </v-expansion-panel-title>
...
// Computed styles
const menuItemStyle = (route) => computed(() => ({
  background: router.currentRoute.value.path === route ? props.currentPathColor : '',
  color: router.currentRoute.value.path === route ? '#fff' : '',
}));

const subMenuItemStyle = (route) => computed(() => ({
  color: router.currentRoute.value.path === route ? props.currentPathColor : '',
}));
</script>
